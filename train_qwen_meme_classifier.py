...
def custom_collate_fn(batch):
    input_ids = torch.nn.utils.rnn.pad_sequence([item['input_ids'] for item in batch], batch_first=True, padding_value=151643)
...
def train():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_update_steps, eta_min=1e-6)
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch in enumerate(loop):
            ...
            loop.set_description(f"Epoch [{epoch+1}/{EPOCHS}] LR: {current_lr:.2e} Loss: {loss.item():.4f}")
    ...
    if accelerator.is_main_process:
        print("Training Complete! The best performing model weights are saved in your results folder.")
