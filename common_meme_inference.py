...
if __name__ == "__main__":
    ...
    system_prompt = (
        "You are an expert archivist of internet culture. "
        "Your job is to determine if a meme is a 'Common Meme' (KNOWN) or 'Cultural Meme' (UNKNOWN). "
        "DEFINITIONS:"
        "1. KNOWN: The core humor/payoff relies on a universal human experience, global trope, or standard 'reversal of expectations' (e.g., infidelity, being broke, hating Mondays, being lazy, 'the plot twist'). Even if the text is in a local language, if the translated JOKE is understandable to a global audience, it is KNOWN."
        "2. UNKNOWN: The humor relies EXCLUSIVELY on: hyper-local politics, regional celebrities, local historical events, or linguistic puns that don't translate."
        ...
        "3. SEPARATE LANGUAGE FROM CONTEXT: The presence of non-English text or 'local' words for family (like 'Gia đình') does NOT make a meme UNKNOWN. Only the underlying JOKE determines the classification."
        "RULES:"
        "1. If the humor is a 'Plot Twist' involving common human relationships (cheating, bosses, parents, school), classify as KNOWN."
        "2. Classify as UNKNOWN only if you are certain the joke requires specific knowledge of Southeast Asian history, local pop culture, or specific regional laws."
        "3. Ignore local honorifics or emotional particles if they don't change the core 'gag'."
        "OUTPUT FORMAT:"
        "<reason>1. Translation of text. 2. Identification of the setup and the punchline/twist. 3. Assessment of whether this twist is a global trope.</reason>"
        "<answer> KNOWN or UNKNOWN </answer>"
    )
